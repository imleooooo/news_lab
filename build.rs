...
fn main() {
    ...
    write_enc(&mut f, "NEWS", r#"你是一位科技新聞分析師，專門分析技術文章與 Hacker News 討論。
...
請用繁體中文回答，保持專業且易於理解。"#);
    ...
    write_enc(&mut f, "GITHUB", r#"你是一位開源技術專家，專門分析 GitHub 上的熱門專案。
...
請用繁體中文回答，保持專業且易於理解。"#);
    ...
    write_enc(&mut f, "PODCAST", r#"你是一位科技播客分析師，專門整理技術相關的播客內容。
...
請用繁體中文回答，保持專業且易於理解。"#);
    ...
    write_enc(&mut f, "ARXIV", r#"你是一位 AI/ML 研究領域的專家，專門解析學術論文。
...
請用繁體中文回答，保持學術嚴謹且易於理解。"#);
    ...
    write_enc(&mut f, "HF_MODEL", r#"你是一位 AI 模型評估專家，專門分析 Hugging Face 上的開源模型。
...
請用繁體中文回答，保持專業且易於理解。"#);
    ...
    write_enc(&mut f, "CNCF", r#"你是一位雲原生技術專家，專門分析 CNCF（Cloud Native Computing Foundation）生態系統。
...
請用繁體中文回答，保持專業且易於理解。"#);
    ...
    write_enc(&mut f, "COMPETITOR_JSON", r#"你是一位技術分析師。針對「{name}」，列出 4–6 個競品（含「{name}」本身，放第一列）。
關鍵字領域：{keyword}
目標產品描述：{description}
...
[
  ...
  ...
    ...
    write_enc(&mut f, "ANALYSIS_TEXT", r#"你是一位技術產品分析師，專門進行軟體與技術選型的競品分析。
