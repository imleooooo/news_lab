...
請用繁體中文回答，保持專業且易於理解。"#,
    ...
    write_enc(
        &mut f,
        "DOCS",
        r#"你是一位技術文件分析師，專門整理和解析技術文件的內容架構。
請針對以下技術文件頁面，提供一份詳盡的繁體中文摘要，結構如下：

1. **文件主旨**
   這份文件介紹什麼技術或產品，解決什麼問題（1–2 句）。

2. **主要涵蓋的內容領域**（條列，5–10 點）
   列出核心章節、主題或功能模塊，讓讀者快速掌握文件的覆蓋範圍。

3. **目標受眾**
   適合哪類讀者（如：初學者、後端開發者、系統管理員、架構師）。

4. **重點概念與操作**（條列，3–5 點）
   文件中特別重要的核心概念、關鍵指令、配置方式或操作流程。

5. **整體評估**
   文件的完整性與深度，以及是否有明顯待補充之處（1–3 句）。

文件 URL：{url}
文件標題：{title}
站內已發現的連結（代表文件可能涵蓋的範圍）：
{nav_links}
文件內容節錄：
{content}

⚠️ 重要：
- 請聚焦在文件實際包含的內容；若節錄不足，可根據 URL、標題與站內連結合理推測，並標示（推測）。
- 若站內連結豐富，請在「主要涵蓋的內容領域」中反映出來。
- 輸出使用條列與分段，避免過長連續段落。
