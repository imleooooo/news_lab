...
① 抓取技術訊號          fetch_radar_signals()
       │
       ▼
 ② 依模式提取 Blip      extract_blips(Project/Method)
       │
       ▼
③ 進階模型審核（×2）    review_and_augment()
       │
       ▼
 ④ GitHub 活躍度校正     check_oss_activity()（僅專案雷達）
       │
       ▼
...
- **專案雷達**：開源/閉源軟體、產品、API 服務、模型服務、平台、工具、框架與函式庫。
- **方法雷達**：方法論、演算法、架構模式、工程實踐、評估/訓練/部署方法。
...
注入對應模式的 prompt，要求 LLM：
1. 專案雷達找出開源/閉源專案、產品、模型服務、平台、工具、框架
2. 方法雷達找出方法論、演算法、架構模式、工程實踐與評估/部署方法
3. 補充 LLM 自身知識中該領域重要項目
4. 為 4 個象限命名（依領域特性調整）
5. 為每個項目判斷成熟度環形
...
> ⚠️ 專案雷達中，閉源服務（如 OpenAI API、Claude API）若業界廣泛使用，仍可列為 `adopt`；
> 「閉源」風險記錄在 `cons` 欄位，不用來壓低環形評級。
>
> 方法雷達中，環形代表研究與工程實務採用成熟度，不代表產品採購成熟度。
...
```
  ...
  [23 × 71 格子，ANSI 上色]
  ─────────────────────────────────────────
  Q3（工具 & 平台）  │ Q4（資料庫 & 基礎設施）

  ▲=開源 ●=閉源   A=Adopt T=Trial S=Assess H=Hold
```

方法雷達不顯示開源/閉源圖例，只顯示成熟度環形。
