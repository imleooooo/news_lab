...
只回傳 JSON，不要加任何其他文字：
{
  ...
  "clusters": [
    ...
    },
    {
      "name": "GitHub Repos",
      "nodes": [
        {"name": "owner/repo", "description": "一句話說明此 repo 的用途"}
      ]
    },
    {
      "name": "框架 & 工具",
...
- 必須包含「核心概念」cluster（重要技術概念、方法論、架構模式）
- 若領域有相關開源專案，建議加入「GitHub Repos」cluster（name 使用 owner/repo 格式）；若沒有也可省略
- relations：5–10 個，描述重要的依賴、使用、繼承、實作等關係
- name：⚠️ 必須使用英文或英文縮寫，≤20 字元（例如 IoT 而非物聯網、Security 而非安全性、Monitoring 而非監控）；name 會作為搜尋關鍵字使用，中文 name 無法搜尋
